...
def CadastroPfisica():
    ...
    send_db = ValidarPfisica.DadosPfisica(cpf=input_cpf, nome=input_nome, cep=input_cep, uf=input_uf, cidade=input_cidade, endereco=input_endereco, numero=input_numero, telefonefixo=input_telefoneum, telefonecelular=input_telefonedois, email=input_email)
    ...
    #---- Retorno da Validação (CEP) ----#
    cep = send_db.cep_valido
    
    if cep == 0:
        formulario_Pfisica.aviso_03.setText("*")
        print('CEP INVALIDO')
        #return CadastroFisico()
    ...
    if (cpf == 0) or (nome == 0) or (cep == 0):
        return CadastroFisico()
    
    
    write_db = WriteDb.Pfisica(cpf=send_db.cpf_ok, nome=send_db.nome_ok, cep=send_db.cep_ok, uf='uf', cidade='cidade', endereco='endereco', numero='numero', telefonefixo='TelefoneFixo', telefonecelular='TelefoneCelular', email='email')
